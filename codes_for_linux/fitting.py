...
parser.add_argument('-o','--outputfolder', type=str, default='results', help='output folder path')
parser.add_argument('-os','--outputsurfix', type=str, default='', help='output file surfix(describe the fitting condition)')
parser.add_argument('-dt','--datatype', type=str, default='ProQEXAFS', help='data type')
parser.add_argument('-bs','--batchsize', type=int, default=-1, help='batch size for fitting')
...
dset,out,report,path=exp_data.run_fit_batch(fitting_param,fitting_range,[0],'feff',save_name=join(foldername,file_name[:-4]+output_surfix),write=True,mpi=False,core=10,batch_size=batch_size)
