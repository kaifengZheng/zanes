...
def data_analysis(group,e0=None, pre_start=-30,pre_end=-10,post_start=20,post_end=900,kweight=2,rbkg=1.5,
    ...
    if plot:
        ...
        ax[0,0].vlines(pre_start+group.e0,0,np.max(group.mu),'b',alpha=0.3,linestyles='--')
        ax[0,0].vlines(pre_end+group.e0,0,np.max(group.mu),'b',alpha=0.3,linestyles='--')
        ax[0,0].vlines(post_start+group.e0,0,np.max(group.mu),'b',alpha=0.3,linestyles='--')
        ax[0,0].vlines(post_end+group.e0,0,np.max(group.mu),'b',alpha=0.3,linestyles='--')
...
def data_analysis_xanes(group,pre_start=-30,pre_end=-10,post_start=20,post_end=900,e0=None,
    ...
    if plot:
        ...
        ax[0].vlines(pre_start+group.e0,0,np.max(group.mu),'b',alpha=0.3,linestyles='--')
        ax[0].vlines(pre_end+group.e0,0,np.max(group.mu),'b',alpha=0.3,linestyles='--')
        ax[0].vlines(post_start+group.e0,0,np.max(group.mu),'b',alpha=0.3,linestyles='--')
        ax[0].vlines(post_end+group.e0,0,np.max(group.mu),'b',alpha=0.3,linestyles='--')
...
def wavelet_transform(group,kweight=2,plot=False):
    ...
    cauchy_wavelet(group.k,group.k**kweight*group.chi*group.kwin,group=group, kweight=0)
    if plot==True:
        ...
        contour=ax[0,1].contourf(X, Y,group.wcauchy_mag.T,cmap='jet',levels=100, vmax=group.wcauchy_mag.max(), vmin=group.wcauchy_mag.min())#,vmin=a0bs(coef).min(),vmax=abs(coef).max())
...
def BF_analysis(group,kweight=3,rrange=[],krange=[]):
    xftr(r=group.r,chir=group.chir,rmin=rrange[0],rmax=rrange[1],dr=0,group=group,window='hanning',kstep=group.k[2]-group.k[1])
    # chiq=interp1d(group.q,group.chiq)(group.k)
    BF_group=deepcopy(group)
    xftf(k=BF_group.q,chi=BF_group.chiq, dk=2,group=BF_group,kweight=0,kmin=krange[0],kmax=krange[1],kstep=group.k[2]-group.k[1],window='hanning')
    # # transform again
    BF_group_2=deepcopy(BF_group)
    xftr(r=BF_group_2.r,chir=BF_group_2.chir, dr=0,group=BF_group_2,kweight=0,rmin=rrange[0],rmax=rrange[1],kstep=group.k[2]-group.k[1],window='hanning')
    ...
    ax[0].plot(BF_group_2.q,BF_group_2.chiq,color='g',label='Fourier transform again')
...
class data_processing_multiOnerun:
    def __init__(self,filename,datatype,read_range=None):
        self.data=[]
    ...
    def read_data(self,filename,datatype='Athena',read_range=None):
    ...
    def process_data(self,data_dict,plot=False):
    ...
    def fit_param(self,**p):
        fit_param_get={}
        for k_SO2 in p['SO2'].keys():
            if p['SO2'][k_SO2]['global']:
                fit_param_get[k_SO2]=param(p['SO2'][k_SO2]['initial'],vary=p['SO2'][k_SO2]['vary'])
            else:
                for i in range(len(self.data)):
                    fit_param_get[f'{k_SO2}_{i}']=param(p['SO2'][k_SO2]['initial'],vary=p['SO2'][k_SO2]['vary'])
        for k_N in p['N'].keys():
            if p['N'][k_N]['global']:
                fit_param_get[k_N]=param(p['N'][k_N]['initial'],vary=p['N'][k_N]['vary'])
            else:
                for i in range(len(self.data)):
                    fit_param_get[f'{k_N}_{i}']=param(p['N'][k_N]['initial'],vary=p['N'][k_N]['vary'])
        for k_dele in p['dele'].keys():
            if p['dele'][k_dele]['global']:
                fit_param_get[k_dele]=param(p['dele'][k_dele]['initial'],vary=p['dele'][k_dele]['vary'])
            else:
                for i in range(len(self.data)):
                    fit_param_get[f'{k_dele}_{i}']=param(p['dele'][k_dele]['initial'],vary=p['dele'][k_dele]['vary'])
        for k_ss2 in p['ss2'].keys():
            if not p['ss2'][k_ss2]['thermal']['type']:
                if p['ss2'][k_ss2]['global']:
                    fit_param_get[k_ss2]=param(p['ss2'][k_ss2]['initial'],vary=p['ss2'][k_ss2]['vary'])
                else:
                    for i in range(len(self.data)):
    ...
    def path_param(self,feff_folder,fitpath_num=[],rules=dict(),temp=[]):
        ...
        for i in range(len(self.data)):
            ...
            for j in range(len(fitpath_num)):
                if rules['SO2'][SO2_k[0]]['global'] and rules['N'][N_k[j]]['global']:
                    s02=f'{N_k[j]}*SO2'
                elif not rules['SO2'][SO2_k[0]]['global'] and not rules['N'][N_k[j]]['global']:
                        s02=f'{N_k[j]}_{i}*SO2_{i}'
                elif rules['SO2'][SO2_k[0]]['global'] and not rules['N'][N_k[j]]['global']:
                    s02=f'{N_k[j]}_{i}*SO2'
                else:
                    s02=f'N*SO2_{i}'
    ...
    def feff_call(path:str,feff_inpname:str)->None:
        feff=feffrunner(folder='feff',feffinp='feff.inp')
        feff.run()

    def feff_path(self,feff_folder):
        with open(f'{feff_folder}/files.dat','r') as f:
            lines=f.readlines()
        num=0
        for i in range(len(lines)):
            if search('^(\s+[A-Za-z]+\d*){8}$',lines[i]):
                num=i
                break
        return pd.read_csv(f'{feff_folder}/files.dat',sep=' +',skiprows=num+1,names=['file','sig2','amp ratio','deg','nlegs','r effective'])
    def feff_rules(self,**parm_dict):
        rules=dict()
        rules['SO2']={}
        for k_SO2 in parm_dict['SO2'].keys():
            rules['SO2'][k_SO2]={"vary":parm_dict['SO2'][k_SO2]["vary"],
        ...
        for k_ss2 in parm_dict['ss2'].keys():
            ...
            if parm_dict['ss2'][k_ss2]['thermal']['type']==False:
                ...
                rules['ss2'][k_ss2].update({"theta":{"vary":theta["vary"],'global':theta['global']}})
            rules['delr']={}
            for dr_k in parm_dict['delr'].keys():
                rules['delr'][dr_k]={"vary":parm_dict['delr'][dr_k]["vary"],
                    "global":parm_dict['delr'][dr_k]['global']
                   }
        rules['dele']={}
        for dele in parm_dict['dele'].keys():
            rules['dele'][dele]={"vary":parm_dict['dele'][dele]["vary"],
                    "global":parm_dict['dele'][dele]['global']
                    }
        return rules

    def run_fit(self,parm_dict,feff_folder,fitpath_num:list,fit_range_param,temp=[],write=False,save_name='fit.out'):
        self.fit_range_param=fit_range_param
        rules=self.feff_rules(**parm_dict)
        pars=self.fit_param(**parm_dict)
        paths=self.path_param(feff_folder,fitpath_num=fitpath_num,rules=rules,temp=temp)
        ...
        for i in range(len(self.data)):
            paths_get=[]
            for j in range(len(fitpath_num)):
                paths_get.append(paths[i*len(fitpath_num)+j])
            # print(paths_get)
            # print('\n')
            dset.append(feffit_dataset(data=self.data[i],pathlist=paths_get,transform=trans))
        out=feffit(pars,dset)
        report=feffit_report(out)
        if write:
            self.write_report(save_name,report)
        return dset,out,report,paths
    

    def write_report(self,filename, out):
    ...
    def plot_fitting_results(self,dset,d_range=[0,-1],klim=[0,20],rlim=[0,6]):
        if d_range[1] is None:
            d_range[1]=len(dset)
        for i in range(len(dset[d_range[0]:d_range[1]])):
            mod=dset[i].model
            dat=dset[i].data
            data_chik=dat.chi*dat.k**dset[i].transform.kweight
            model_chik=mod.chi*mod.k**dset[i].transform.kweight
            fig,ax=plt.subplots(1,2)
            ax[0].plot(dat.k,data_chik,label=self.data[d_range[0]+i].label)
            ...
            ax[0].set_xlim(klim[0],klim[1])
            ax[1].set_xlim(rlim[0],rlim[1])
            ax[0].legend(frameon=False)
            plt.show()
    def write_sorted_report(self,out,param_dict,path):     
        reff=[]
        for p in path:
            reff.append(p.reff)
        params={}
        for k in param_dict.keys():
            for k2 in param_dict[k].keys():
                if param_dict[k][k2]['global']:
                    params[k2]={'value':out.params[k2].value,
                                'stderr':out.params[k2].stderr}
                    print(f"GLOBEL: {k2}:{params[k2]['value']}+/-{params[k2]['stderr']}") 
                    # else:
                    #     params[k][k2]['value'].append(out.params[k2].value)
                    #     params[k][k2]['stderr'].append(out.params[k2].stderr)
                else:
                        num_k = len([key for key in out.params.keys() if k2 in key])
                        for j in range(num_k):
                            if k2 not in params.keys(): 
                                params[k2]={f'{k2}_{j}':{'value':out.params[f'{k2}_{j}'].value,
                                                            'stderr':out.params[f'{k2}_{j}'].stderr}}
                            else:
                                params[k2][f'{k2}_{j}']={'value':out.params[f'{k2}_{j}'].value,
                                                            'stderr':out.params[f'{k2}_{j}'].stderr}
        for i in range(len(self.data)):
            ...
            for k2i in range(len(k2)):
                if param_dict['delr'][k2[k2i]]['global']:
                    ...
                    params['r']={f'r_{ele}':{'value':params[f"delr_{ele}"]['value']+reff[k2i],'stderr':params['delr'][k2[k2i]]['stderr']}}
                else:
                    ele=k2[k2i].split('_')[1]
                    if f'r_{ele}' not in params.keys():
                        params[f'r_{ele}']={f'r_{ele}_{i}':{'value':params[f"delr_{ele}"][f'delr_{ele}_{i}']['value']+reff[k2i],'stderr':params[f'delr_{ele}'][f'delr_{ele}_{i}']['stderr']}}
                    else:
                        ele=k2[k2i].split('_')[1]
                        params[f'r_{ele}'].update({f'r_{ele}_{i}':{'value':params[f"delr_{ele}"][f'delr_{ele}_{i}']['value']+reff[k2i],'stderr':params[f'delr_{ele}'][f'delr_{ele}_{i}']['stderr']}})
        return params 

class data_processing_multiloop:
    def __init__(self,filename,datatype,read_range=None):
        self.data=[]
        self.filename=filename
        self.datatype=datatype
    ...
    def read_data(self,filename,datatype='Athena',read_range=None):
        if isinstance(read_range,(int,np.int16,np.int32,np.int64)) and read_range is not None:
            scan=0
        elif isinstance(read_range,(list,np.ndarray)) and read_range!=[]:
            scan_begin=read_range[0]
            scan_end=read_range[1]
        elif read_range is None:
            pass
        ...
        if datatype=='Athena':
            project_group=read_athena(filename)
            if isinstance(read_range,(list,np.ndarray)) and read_range is not None:
                for key in project_group:
                    self.data.append(project_group[key])
                    scan+=1
                    if scan==read_range:
        ...
        if datatype=='csv':
            ...
            if not isinstance(read_range,(list,np.ndarray)) and read_range is not None:
                for i in range(1,len(keys)):
                    ...
                    if scan==read_range:
                        break
            elif isinstance(read_range,(list,np.array)) and read_range!=[]:
                for i in range(scan_begin,scan_end):
                    self.data.append(Group(energy=np.array(data_get['E']),mu=np.array(data_get[keys[i]]),label=keys[i]))
            elif read_range is None:
                for i in range(1,len(keys)):
    ...
    def read_data_raw(self,filename):
        with open(filename) as file1:
            data=file1.readlines()
        dict_data=dict()
        for i in range(len(data)):
    ...
    def fit_param(self,index,**p):
        ...
        for k_SO2 in p['SO2'].keys():
            if p['SO2'][k_SO2]['global']:
                ...
                fit_param_get[f'{k_SO2}_{index}']=param(p['SO2'][k_SO2]['initial'],vary=p['SO2'][k_SO2]['vary'])
        for k_N in p['N'].keys():
            if p['N'][k_N]['global']:
                fit_param_get[k_N]=param(p['N'][k_N]['initial'],vary=p['N'][k_N]['vary'])
            else:
                fit_param_get[f'{k_N}_{index}']=param(p['N'][k_N]['initial'],vary=p['N'][k_N]['vary'])
        for k_dele in p['dele'].keys():
            if p['dele'][k_dele]['global']:
                fit_param_get[k_dele]=param(p['dele'][k_dele]['initial'],vary=p['dele'][k_dele]['vary'])
            else:
                fit_param_get[f'{k_dele}_{index}']=param(p['dele'][k_dele]['initial'],vary=p['dele'][k_dele]['vary'])
        for k_ss2 in p['ss2'].keys():
            if not p['ss2'][k_ss2]['thermal']['type']:
                if p['ss2'][k_ss2]['global']:
                    fit_param_get[k_ss2]=param(p['ss2'][k_ss2]['initial'],vary=p['ss2'][k_ss2]['vary'])
                else:
                    fit_param_get[f'{k_ss2}_{index}']=param(p['ss2'][k_ss2]['initial'],vary=p['ss2'][k_ss2]['vary'])
            if p['ss2'][k_ss2]['thermal']['type']=='Einstein':
                if p['ss2'][k_ss2]['global']:
                    suffix=k_ss2.split('_')[1]
                    fit_param_get[k_ss2]=param(p['ss2'][k_ss2]['initial'],vary=p['ss2'][k_ss2]['vary'])
            ...
            for dr_k in p['delr'].keys():
                if p['delr'][dr_k]['global']:
                    ...
                    fit_param_get[f'{dr_k}_{index}']=param(p['delr'][dr_k]['initial'],vary=p['delr'][dr_k]['vary'])
        pars=param_group(**fit_param_get)  
        return pars     
    def path_param(self,feff_folder:str,fitpath_num=[],rules=dict(),temp=[],index=0)->list:
        #for now, only consider the first path
        ...
        for j in range(len(fitpath_num)):
            if rules['SO2'][SO2_k[0]]['global'] and rules['N'][N_k[j]]['global']:
                s02=f'{N_k[j]}*SO2'
                    ...
                    s02=f'{N_k[j]}_{index}*SO2_{index}'
            elif rules['SO2'][SO2_k[0]]['global'] and not rules['N'][N_k[j]]['global']:
                s02=f'{N_k[j]}_{index}*SO2'
            else:
                s02=f'N*SO2_{index}'
            if rules['dele'][dele_k[j]]['global']:
                    dele=dele_k[j]
            else:
                    dele=f'{dele_k[j]}_{index}'
            if not rules['ss2'][ss2_k[j]]['thermal']:
                if rules['ss2'][ss2_k[j]]['global']:
                        ss2=ss2_k[j]
                else:
                        ss2=f'{ss2_k[j]}_{index}'
            else: 
                suffix=ss2_k[j].split('_')[1]
                if rules['ss2'][ss2_k[j]]['thermal']=='Einstein':
                    ss2=f'{ss2_k[j]}+sigma2_eins({temp[index]},theta_{suffix})'
                elif rules['ss2'][ss2_k[j]]['thermal']=='Debye':
                    ss2=f'{ss2_k[j]}+sigma2_debye({temp[index]},theta_{suffix})'   
            if rules['delr'][dr_k[j]]['global']:
                delr=dr_k[j]
            if not rules['delr'][dr_k[j]]['global']:
                delr=f'{dr_k[j]}_{index}'
    ...
    def feff_rules(self,**parm_dict):
        ...
        for k_ss2 in parm_dict['ss2'].keys():
            ...
            if not parm_dict['ss2'][k_ss2]['thermal']['type']:
                rules['ss2'][k_ss2]={"vary":parm_dict['ss2'][k_ss2]["vary"],
                    "global":parm_dict['ss2'][k_ss2]['global'],
                    "thermal":False
                    }
        ...
        return rules
    def run_fit_One(self,parm_dict,feff_folder,fitpath_num,fit_range_param,i=0):
        pars=self.fit_param(index=i,**parm_dict)
        rules=self.feff_rules(**parm_dict)
        paths=self.path_param(feff_folder,fitpath_num,rules=rules,index=i)
        trans= feffit_transform(**fit_range_param)
        dset=[]
        paths_get=[]
        for j in range(len(fitpath_num)):
            paths_get.append(paths[j])
            # print(paths_get)
            # print('\n')
            dset.append(feffit_dataset(data=self.data[i],pathlist=paths_get,transform=trans))
        dset=feffit_dataset(data=self.data[i],pathlist=paths_get,transform=trans)
        # print(pars.items())
        out=feffit(pars,dset)
        report=feffit_report(out)
        return dset,out,report
    def run_fit_multi(self,parm_dict,feff_folder,fitpath_num,fit_range_param,write=False,save_name='fit.out',mpi=False,core=1):
        dsets,outs,reports=[],[],[]
        if not mpi:
            for i in range(len(self.data)):
                dset,out,report=self.run_fit_One(parm_dict,feff_folder,fitpath_num,fit_range_param,i=i)
                dsets.append(dset)
                outs.append(out)
                reports.append(report)
        if mpi and core>1:
            run=partial(self.run_fit_One,parm_dict,feff_folder,fitpath_num,fit_range_param)
            my_iter=range(len(self.data))
            with confu.ThreadPoolExecutor(max_workers=core) as executor:
                jobs=tqdm(executor.map(run,my_iter),total=len(self.data))
            for job in jobs:
                dset,out,report=job
                dsets.append(dset)
                outs.append(out)
                reports.append(report)
        self.write_report(save_name,reports)   
        return dsets,outs,reports


    def write_report(self,filename, outs):
        "write report to file"
        try:
            f = open(filename, 'w')
            i=0
            for out in outs:
                f.write(f"=====================FIT {i}th data=====================\n")
                f.write(out)
                f.write('\n')
                f.write('\n')
            f.close()
        except:
            raise ValueError( 'could not write %s' % filename)

    def clean_fit_param(self):
        self.fit_param_get={}

    def plot_fitting_results(self,dset,d_range=[0,-1],klim=[0,20],rlim=[0,5]):
        if d_range[1] is None:
            d_range[1]=len(dset)
        for i in range(len(dset[d_range[0]:d_range[1] ])):
            mod=dset[i].model
...
class data_processing_minibatch:
     ...
     def feff_rules(self,**parm_dict):
        ...
        return rules
     def fit_one_batch(self,parm_dict:dict,feff_folder:str,fitpath_num:int,
                       fit_range_param:dict,batch_size:int,batch_index:int):
        dset=[]
        true_batch_size=0
        ...
        if batch_index==batch_num:
            ...
            paths=self.path_param_batch(feff_folder,fitpath_num,rules=rules,batch_size=batch_size_last)
        else:
     ...
     def run_fit_batch(self,param_dict:dict,
        ...
        if mpi==False:
            for i in tqdm(range(batch_num),total=batch_num):
                # print(i)
                dset,out,report,batch_num,paths=self.fit_one_batch(param_dict,
                                                             feff_folder,
        ...
        if mpi==True and core>1:
            run=partial(self.fit_one_batch,param_dict,
                                           feff_folder,
                                           fitpath_num,
                                           fit_range_param,
                                           batch_size)
     ...
     def write_sorted_report(self,out:list,report:list,param_dict,path,batch_size=None,batch_num=None):
        ...
        for i in range(batch_num):
            for k in param_dict.keys():
                for k2 in param_dict[k].keys():
                    if param_dict[k][k2]['global']:
                        if k2 not in params.keys():
                            params[k2]={'value':[out[i].params[k2].value],
        ...
        for i in range(num):
            ...
            for k2i in range(len(k2)):
                if param_dict['delr'][k2[k2i]]['global']:
                    ...
                    if f'r_{ele}' not in params.keys():
                        # print(reff)
                        params[f'r_{ele}']={f'r_{ele}_{i}':{'value':params[f"delr_{ele}"][f'delr_{ele}_{i}']['value']+reff[k2i],
     ...
     def write_fitted_data(self,dset,filename:str,foldername:str,suffix:str)->None:
        ...
        for i in range(len(dset)):
            for j in range(len(dset[i])):
                data_k[f'{i*j+j}']=np.round(interp1d(dset[i][j].data.k,dset[i][j].data.chi* dset[i][j].data.k**dset[i][j].transform.kweight)(kdata),6)
                data_r[f'{i*j+j}']=np.round(dset[i][j].model.r,6)
                fitted_k[f'{i*j+j}']=np.round(interp1d(dset[i][j].model.k,dset[i][j].model.chi*dset[i][j].model.k**dset[i][j].transform.kweight)(kdata),6)
        ...
        data_k_table.to_csv(join(foldername,f'{file[:-4]}{suffix}_kdata.csv'),index=False)
        data_r_table.to_csv(join(foldername,f'{file[:-4]}{suffix}_rdata.csv'),index=False)
        fitted_k_table.to_csv(join(foldername,f'{file[:-4]}{suffix}_kfitted.csv'),index=False)
        fitted_r_table.to_csv(join(foldername,f'{file[:-4]}{suffix}_rfitted.csv'),index=False)
        ...
        for rep in report:
            ...
            for i in range(len(lines)):
                if match('\ +r-factor\ +=\ +(\d+.?\d+)',lines[i]):
                    evaluation['r_factor'].append(np.round(np.float32(match('\ +r-factor\ +=\ +(\d+.?\d+)',lines[i]).group(1)),6))
     ...
     def plot_fitting_batch_results(self,batch_index,dset,kweight,d_range=[0,-1]):
        for i in range(len(dset[batch_index][d_range[0]:d_range[1] ])):
            mod=dset[batch_index][i].model
            dat=dset[batch_index][i].data
            data_chik=dat.chi*dat.k**dset[batch_index][i].transform.kweight
            model_chik=mod.chi*mod.k**dset[batch_index][i].transform.kweight
            fig,ax=plt.subplots(1,2)
            ax[0].plot(dat.k,data_chik,label=self.data[d_range[0]+i].label)
            ax[0].plot(mod.k,model_chik)
            ax[0].plot(dat.k,dat.kwin)
